 ...
 */
public class ButLast implements BuildInFunction {
    ...
    public MValue run(Runtime ctx, MValue[] args) throws Error {
        try {
            if (args[0] instanceof MLiteral){
                ...
                return new MLiteral(value.substring(0, value.length() - 1));
            }
            ...
            else throw new RuntimeError("The type of arguments of function `butlast` is incorrect.");
        }
        catch (RuntimeException e){
            throw new RuntimeError("Command `butlast` cannot be applied for NULL list.");
        }
    }
}
